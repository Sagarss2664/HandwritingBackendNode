...
app.post('/api/register', async (req, res) => {
  try {
    ...
    // Send email to all admins - IMPORTANT: Make this synchronous with better logging
    const admins = await Admin.find({ isActive: true });
    const adminEmails = admins.map(admin => admin.email);
    ...
    console.log('🔍 REGISTRATION DEBUG:', {
      ...
      brevoApiKey: BREVO_API_KEY ? 'Set' : 'Not Set',
    ...
    if (adminEmails.length > 0 && BREVO_API_KEY) {
      try {
        const emailSubject = 'New User Registration - Approval Required';
        const emailHtml = `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <hr>
            <p style="font-size: 12px; color: #666;">This email was automatically generated by Grapho Genius system.</p>
          </div>
        `;

        console.log('📨 Attempting to send admin notification emails...');
        
        // Use the same function that works for other emails
        emailResult = await sendEmailViaBrevo(adminEmails[0], emailSubject, emailHtml);
        
        if (emailResult) {
          console.log('✅ Admin notification email sent successfully:', emailResult);
          
          // Send to remaining admins if more than one (background)
          if (adminEmails.length > 1) {
            setTimeout(async () => {
              try {
                await sendEmailToMultiple(adminEmails.slice(1), emailSubject, emailHtml);
                console.log(`✅ Sent emails to ${adminEmails.length - 1} additional admins`);
              } catch (bgError) {
                console.error('❌ Background email sending failed:', bgError.message);
              }
            }, 1000);
          }
        } else {
          console.log('⚠️ Email sending returned null (check Brevo logs)');
        }
      } catch (emailError) {
        console.error('❌ Email sending failed in registration:', emailError);
        // Don't fail the registration just because email failed
      }
    } else {
      if (!BREVO_API_KEY) {
        console.log('⚠️ BREVO_API_KEY not configured, skipping admin emails');
      }
      if (adminEmails.length === 0) {
        console.log('⚠️ No active admins found, skipping admin emails');
      }
    }
    ...
    res.status(201).json({
      ...
      adminNotification: {
        attempted: adminEmails.length > 0 && !!BREVO_API_KEY,
